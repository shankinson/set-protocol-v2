...
contract AaveV3LeverageModule is ModuleBase, ReentrancyGuard, Ownable, IModuleIssuanceHook {
    ...
    struct ActionInfo {
        ...
        IPool pool;                              // Pool instance, we grab this everytime since it's best practice not to store
    ...
    // Used to fetch pool address. This contract is immutable and its address will never change.
     ...
     * @param _poolAddressesProvider     Address of Aave PoolAddressProvider
     */
    constructor(
        ...
        ModuleBase(_controller)
    {
        ...
        IPoolDataProvider _poolDataProvider = IPoolDataProvider(_poolAddressesProvider.getPoolDataProvider());
        poolDataProvider = _poolDataProvider;

        IPoolDataProvider.TokenData[] memory reserveTokens = _poolDataProvider.getAllReservesTokens();
        for(uint256 i = 0; i < reserveTokens.length; i++) {
            (address aToken, , address variableDebtToken) = _poolDataProvider.getReserveTokensAddresses(reserveTokens[i].tokenAddress);
        ...
        onlyManagerAndValidSet(_setToken)
    {
        ...
        _borrow(leverInfo.setToken, leverInfo.pool, leverInfo.borrowAsset, leverInfo.notionalSendQuantity);
        ...
        _supply(leverInfo.setToken, leverInfo.pool, _collateralAsset, postTradeCollateralQuantity);
        ...
        onlyManagerAndValidSet(_setToken)
    {
        ...
        _withdraw(deleverInfo.setToken, deleverInfo.pool, _collateralAsset, deleverInfo.notionalSendQuantity);
        ...
        _repayBorrow(deleverInfo.setToken, deleverInfo.pool, _repayAsset, repayQuantity);
        ...
        returns (uint256)
    {
        ...
        _repayBorrow(deleverInfo.setToken, deleverInfo.pool, _repayAsset, notionalRepayQuantity);
    ...
    function addUnderlyingToReserveTokensMapping(IERC20 _underlying) external {
        ...
        (,,,,,,,, bool isActive,) = poolDataProvider.getReserveConfigurationData(address(_underlying));
     ...
     */
    function _supply(ISetToken _setToken, IPool _pool, IERC20 _asset, uint256 _notionalQuantity) internal {
        _setToken.invokeApprove(address(_asset), address(_pool), _notionalQuantity);
        _setToken.invokeSupply(_pool, address(_asset), _notionalQuantity);
    }

    /**
     * @dev Invoke withdraw from SetToken using AaveV3 library. Burns aTokens and returns underlying to SetToken.
     */
    function _withdraw(ISetToken _setToken, IPool _pool, IERC20 _asset, uint256 _notionalQuantity) internal {
        _setToken.invokeWithdraw(_pool, address(_asset), _notionalQuantity);
    }

    /**
     * @dev Invoke repay from SetToken using AaveV3 library. Burns DebtTokens for SetToken.
     */
    function _repayBorrow(ISetToken _setToken, IPool _pool, IERC20 _asset, uint256 _notionalQuantity) internal {
        _setToken.invokeApprove(address(_asset), address(_pool), _notionalQuantity);
        _setToken.invokeRepay(_pool, address(_asset), _notionalQuantity, BORROW_RATE_MODE);
    }
    ...
    function _repayBorrowForHook(ISetToken _setToken, IERC20 _asset, uint256 _notionalQuantity) internal {
        _repayBorrow(_setToken, IPool(poolAddressesProvider.getPool()), _asset, _notionalQuantity);
    }

    /**
     * @dev Invoke borrow from the SetToken using AaveV3 library. Mints DebtTokens for SetToken.
     */
    function _borrow(ISetToken _setToken, IPool _pool, IERC20 _asset, uint256 _notionalQuantity) internal {
        _setToken.invokeBorrow(_pool, address(_asset), _notionalQuantity, BORROW_RATE_MODE);
    }
    ...
    function _borrowForHook(ISetToken _setToken, IERC20 _asset, uint256 _notionalQuantity) internal {
        _borrow(_setToken, IPool(poolAddressesProvider.getPool()), _asset, _notionalQuantity);
    }
    ...
    function _addUnderlyingToReserveTokensMapping(IERC20 _underlying) internal {
        (address aToken, , address variableDebtToken) = poolDataProvider.getReserveTokensAddresses(address(_underlying));
    ...
    function _updateUseReserveAsCollateral(ISetToken _setToken, IERC20 _asset, bool _useAsCollateral) internal {
        ...
        ---------------------------------------------------------------------------------------------------------------------
        */
        (,,,,,,,,bool usageAsCollateralEnabled) = poolDataProvider.getUserReserveData(address(_asset), address(_setToken));
        if (
    ...
    function _validateNewCollateralAsset(ISetToken _setToken, IERC20 _asset) internal view {
        ...
        (address aToken, , ) = poolDataProvider.getReserveTokensAddresses(address(_asset));
        require(address(underlyingToReserveTokens[_asset].aToken) == aToken, "Invalid aToken address");

        ( , , , , , bool usageAsCollateralEnabled, , , bool isActive, bool isFrozen) = poolDataProvider.getReserveConfigurationData(address(_asset));
        // An active reserve is an alias for a valid reserve on Aave.
        ...
        require(isActive, "Invalid aave reserve");
        // A frozen reserve doesn't allow any new supply, borrow or rate swap but allows repayments, liquidations and withdrawals
    ...
    function _validateNewBorrowAsset(ISetToken _setToken, IERC20 _asset) internal view {
        ...
        ( , , address variableDebtToken) = poolDataProvider.getReserveTokensAddresses(address(_asset));
        require(address(underlyingToReserveTokens[_asset].variableDebtToken) == variableDebtToken, "Invalid variable debt token address");

        (, , , , , , bool borrowingEnabled, , bool isActive, bool isFrozen) = poolDataProvider.getReserveConfigurationData(address(_asset));
        require(isActive, "Invalid aave reserve");
        require(!isFrozen, "Frozen aave reserve");
